...
public class SpaceUserServiceImpl extends ServiceImpl<SpaceUserMapper, SpaceUser>
    ...
    public List<SpaceUserVO> getSpaceUserVOList(List<SpaceUser> spaceUsers) {
        ...
        // 对象列表 => 封装对象列表
        List<SpaceUserVO> userVOList = spaceUsers.stream().map(SpaceUserVO::objToVo).collect(Collectors.toList());
        // 1. 收集需要关联查询的用户 ID 和空间 ID
        Set<Long> userIds = userVOList.stream().map(SpaceUserVO::getUserId).collect(Collectors.toSet());
        Set<Long> spaceIds = userVOList.stream().map(SpaceUserVO::getSpaceId).collect(Collectors.toSet());
        // 2. 批量查询用户和空间
        Map<Long, List<User>> userIdUserListMap = userService.listByIds(userIds).stream()
                .collect(Collectors.groupingBy(User::getId));
        Map<Long, List<Space>> spaceIdSpaceListMap = spaceService.listByIds(spaceIds).stream()
                .collect(Collectors.groupingBy(Space::getId));
        userVOList.forEach(spaceUserVO -> {
