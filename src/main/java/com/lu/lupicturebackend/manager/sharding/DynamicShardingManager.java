package com.lu.lupicturebackend.manager.sharding;
...
public class DynamicShardingManager {
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "lu_picture." + tableName) // 确保前缀合法
    ...
    public void createSpacePictureTable(Space space) {
        ...
        if (space.getSpaceType() == SpaceTypeEnum.TEAM.getValue() && space.getSpaceLevel() == SpaceLevelEnum.FLAGSHIP.getValue()) {
            ...
            try {
                SqlRunner.db().update(createTableSql); // 这里需要在配置文件开启sql运行器为true
                // 更新分表
                updateShardingTableNodes();
            } catch (Exception e) {
                e.printStackTrace();
                log.error("创建图片空间分表失败：空间id:{}", spaceId);
            }
        }
    }
